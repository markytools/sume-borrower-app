/****************************************************************************
** Meta object code from reading C++ file 'infopopup.h'
**
** Created by: The Qt Meta Object Compiler version 67 (Qt 5.9.0)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "../infopopup.h"
#include <QtCore/qbytearray.h>
#include <QtCore/qmetatype.h>
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'infopopup.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 67
#error "This file was generated using the moc from 5.9.0. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
QT_WARNING_PUSH
QT_WARNING_DISABLE_DEPRECATED
struct qt_meta_stringdata_Infopopup_t {
    QByteArrayData data[7];
    char stringdata0[112];
};
#define QT_MOC_LITERAL(idx, ofs, len) \
    Q_STATIC_BYTE_ARRAY_DATA_HEADER_INITIALIZER_WITH_OFFSET(len, \
    qptrdiff(offsetof(qt_meta_stringdata_Infopopup_t, stringdata0) + ofs \
        - idx * sizeof(QByteArrayData)) \
    )
static const qt_meta_stringdata_Infopopup_t qt_meta_stringdata_Infopopup = {
    {
QT_MOC_LITERAL(0, 0, 9), // "Infopopup"
QT_MOC_LITERAL(1, 10, 15), // "on_Back_clicked"
QT_MOC_LITERAL(2, 26, 0), // ""
QT_MOC_LITERAL(3, 27, 17), // "on_Delete_clicked"
QT_MOC_LITERAL(4, 45, 19), // "setReturnedCheckBox"
QT_MOC_LITERAL(5, 65, 21), // "borrowedEquipmentData"
QT_MOC_LITERAL(6, 87, 24) // "on_exportToExcel_clicked"

    },
    "Infopopup\0on_Back_clicked\0\0on_Delete_clicked\0"
    "setReturnedCheckBox\0borrowedEquipmentData\0"
    "on_exportToExcel_clicked"
};
#undef QT_MOC_LITERAL

static const uint qt_meta_data_Infopopup[] = {

 // content:
       7,       // revision
       0,       // classname
       0,    0, // classinfo
       4,   14, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       0,       // signalCount

 // slots: name, argc, parameters, tag, flags
       1,    0,   34,    2, 0x08 /* Private */,
       3,    0,   35,    2, 0x08 /* Private */,
       4,    1,   36,    2, 0x08 /* Private */,
       6,    0,   39,    2, 0x08 /* Private */,

 // slots: parameters
    QMetaType::Void,
    QMetaType::Void,
    QMetaType::Void, QMetaType::QObjectStar,    5,
    QMetaType::Void,

       0        // eod
};

void Infopopup::qt_static_metacall(QObject *_o, QMetaObject::Call _c, int _id, void **_a)
{
    if (_c == QMetaObject::InvokeMetaMethod) {
        Infopopup *_t = static_cast<Infopopup *>(_o);
        Q_UNUSED(_t)
        switch (_id) {
        case 0: _t->on_Back_clicked(); break;
        case 1: _t->on_Delete_clicked(); break;
        case 2: _t->setReturnedCheckBox((*reinterpret_cast< QObject*(*)>(_a[1]))); break;
        case 3: _t->on_exportToExcel_clicked(); break;
        default: ;
        }
    }
}

const QMetaObject Infopopup::staticMetaObject = {
    { &QWidget::staticMetaObject, qt_meta_stringdata_Infopopup.data,
      qt_meta_data_Infopopup,  qt_static_metacall, nullptr, nullptr}
};


const QMetaObject *Infopopup::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->dynamicMetaObject() : &staticMetaObject;
}

void *Infopopup::qt_metacast(const char *_clname)
{
    if (!_clname) return nullptr;
    if (!strcmp(_clname, qt_meta_stringdata_Infopopup.stringdata0))
        return static_cast<void*>(const_cast< Infopopup*>(this));
    if (!strcmp(_clname, "CustomQtUI"))
        return static_cast< CustomQtUI*>(const_cast< Infopopup*>(this));
    return QWidget::qt_metacast(_clname);
}

int Infopopup::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = QWidget::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        if (_id < 4)
            qt_static_metacall(this, _c, _id, _a);
        _id -= 4;
    } else if (_c == QMetaObject::RegisterMethodArgumentMetaType) {
        if (_id < 4)
            *reinterpret_cast<int*>(_a[0]) = -1;
        _id -= 4;
    }
    return _id;
}
QT_WARNING_POP
QT_END_MOC_NAMESPACE
